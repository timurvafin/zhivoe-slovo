...
«Живое слово» (zhivoe-slovo) — синтез идей из восьми открытых скиллов, каждый под лицензией MIT. Тексты этого репозитория написаны заново (оригинальная редакция и компоновка), но идеи и подходы заимствованы. Спасибо авторам.
...
| [petergyang/no-ai-slop](https://github.com/petergyang/no-ai-slop) | Peter Yang | псевдо-инсайт, пустой вывод в хвосте предложения, режиссура восприятия, заходы-анонсы и пустые усилители, отрицательное перечисление, правило «концовку-афоризм удалить, не переписать», формат аудита «паттерн + цитата + фикс», пропорциональность правки, тест-гороскоп на уровне предложения |
